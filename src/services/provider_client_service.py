...
async def get_provider_clients_by_service_id(
    session: Session, service_id: int
) -> list[ProviderClient]:
    result = await session.execute(
        select(ProviderClient).where(ProviderClient.service_id == service_id)
    )
...
async def get_provider_with_code_and_service_id(
    session: Session, client_code: str, service_id: int
) -> ProviderClient:
    stmt = select(ProviderClient).where(
        ProviderClient.client_code == client_code,
        ProviderClient.service_id == service_id,
    )
    result = await session.execute(stmt)
    provider_client = result.scalars().first()
    if provider_client is None:
        raise HTTPException(status_code=404, detail="Provider Client not found")
    return provider_client
...
async def delete_provider_client(
    session: Session, provider_client_id: int
) -> ProviderClient:
    provider_client = await session.get(ProviderClient, provider_client_id)
    if provider_client is None:
...
async def create_provider_client(
    ...
    if existing_provider_client is not None:
        raise HTTPException(
            status_code=400, detail="Client code already exists for this service"
        )
